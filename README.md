...
- **Farben mischen**: zwei Farben in den Mischtopf, heraus kommt die dritte — auf dem Malkasten-Kreis gerechnet, also Blau + Gelb = Grün
...
- **Klebeband**: eine Form aufziehen und nur noch innen (Schablone) oder nur noch außen herum (abkleben) malen — jedes Werkzeug hält sich daran, auch der Radierer
...
- **Drucken kann dreierlei**: das Bild, eine **Grußkarte** zum Falten (Bild vorne, innen leer) oder den **Filmstreifen** — die leere Seite, vier Schritte dazwischen und das fertige Bild auf einem Blatt
...
- **Sticker-Bogen drucken**: die eigenen Sticker zum Ausschneiden
...
Die Test-Suite umfasst 715 Tests in 67 Dateien:
...
- `pixie_palette.dart` — **die eine** Farbquelle. Jeder UI-Ton leitet sich hiervon ab, und ein Test hält das fest (siehe unten). (Die Malfarben in `util/color_utils.dart` sind bewusst getrennt: das ist Inhalt, keine Oberfläche.)
...
**Was ein Kind nicht verlieren darf** (seit v8.7): Vier Wege, auf denen Arbeit verschwand, und alle vier taten es **stumm** — was die schlimmste Sorte ist, weil weder das Kind noch die Eltern merken, dass etwas passiert ist.

Ein zweiter Finger auf dem Bildschirm verwarf den laufenden Strich. Das war als Handballen-Schutz gedacht, trifft aber bei einem Kind meistens einen aufgelegten Daumen oder die Hand des Geschwisterkinds. Jetzt gilt: **was schon zu sehen ist, wird festgeschrieben; was nur vorgemerkt ist, verfällt.** Ein Strich und eine gezogene Form landen auf der Ebene — das kostet einen Tipp, um es zurückzunehmen, während Verwerfen gar nicht zurückzunehmen war. Ein vorgemerkter Sticker hat noch nichts gezeichnet und fällt weiter weg.

Rückgängig war auf der großen Leinwand **genau drei Schritte** tief: ein Schritt war ein Klon der ganzen Malebene (2048 × 1536 × 4 = 12,58 MB), und mehr passte nicht ins 48-MB-Budget. Der Stapel merkt sich jetzt nur den **Ausschnitt**, den eine Operation gleich überschreibt. Dieselben 48 MB tragen damit Dutzende Schritte, und die Decke bleibt, wo sie war — alte Android-7-Geräte haben nicht mehr zu verschenken. Was sich nicht ehrlich eingrenzen lässt, kostet weiterhin das ganze Bild: eine Flutfüllung läuft bis zur nächsten Linie, ein Strich mit Zauber-Spiegel steht an vier Stellen gleichzeitig. **Das Rechteck ist lieber zu groß als zu klein** — zu klein hinterlässt einen Saum alter Pixel in einer Ecke, in die niemand schaut, und das ist die einzige Art, wie der Umbau still danebengehen kann. `test/undo_patch_test.dart` macht deshalb jedes Werkzeug zweimal rückgängig, über den Flicken und über eine Vollkopie, und vergleicht Pixel für Pixel.

Ein App-Wechsel kürzte die Historie auf einen Schritt. Der Grund war gut — im Hintergrund entscheidet Android, welchen Prozess es sich holt — trifft aber das falsche Kind: eine Benachrichtigung, ein Elternteil, das kurz das Tablet nimmt. Getrimmt wird jetzt auf 8 MB, und die Pointe ist, dass der eine Schritt, der früher übrig blieb, mit 12,58 MB **mehr** Speicher war als das ganze neue Budget.
...
**Das Klebeband ist kein Werkzeug** (seit v9.2): Es ist ein *Zustand des Papiers*. Deshalb hält sich jedes Werkzeug daran — Stift, Sticker, Buchstaben, Formen, Eimer, Zauberstab und ausdrücklich der Radierer, denn „alles wegwischen und das Abgedeckte bleibt stehen" ist der Sinn von Klebeband. Genau eine Stelle entscheidet darüber (`_masked` in `op_apply.dart`), damit die Kante für alles dieselbe ist. Zwei Entscheidungen dahinter sind nicht offensichtlich: **Das Band gehört zur Operation, nicht ins Op-Log als eigener Schritt** — es ist keine Station der Geschichte, sondern eine Bedingung, unter der die nächsten Schritte passiert sind, und als eigener Op bräche es die Regel, auf der der Replay-Cursor steht (ein Op, ein Rückgängig-Schritt); der Zeitraffer endete still einen Strich zu früh. Und **der Eimer läuft weiter bis zur nächsten Linie und wird erst danach beschnitten**: das Band als Wand *in* den Isolate zu geben würde ändern, was der Eimer tut, statt was er bedeckt. Kleben selbst ist kein Rückgängig-Schritt, weil dabei nichts gemalt wird.

**Gemischt wird auf dem Malkasten-Kreis** (seit v9.2): Auf dem Bildschirm sind die Grundfarben Rot, *Grün* und Blau; Gelb ist dort in ein Sechstel des Kreises gequetscht, und Blau plus Gelb ergibt Grau — die Antwort, an die sich jeder Erwachsene als „das stimmt doch nicht" erinnert. `mixPaint` (`util/color_utils.dart`) trägt die Farbtöne deshalb auf den Rot-Gelb-Blau-Kreis hinüber, mischt dort und trägt sie zurück. Grau, Weiß und Schwarz verschieben nur die Helligkeit (Rot + Weiß ist Rosa), und weit auseinanderliegende Farben trüben sich — wer alles zusammenrührt, bekommt Matsch. Das ist die einzige Stelle der App, deren Ergebnis ein Kind an etwas außerhalb der App nachprüfen kann, nämlich am eigenen Farbkasten.
...
Damit es nicht ein drittes Mal auseinanderläuft, prüft `test/palette_discipline_test.dart` die Behauptung, statt sie zu wiederholen: jede Datei, die noch einen rohen Wert halten darf, steht dort **mit dem Grund**. Eine Datei hinzuzufügen ist erlaubt — niemand soll nie wieder einen Hex-Wert schreiben, nur niemand mehr, ohne zu sagen warum. Der Test läuft in beide Richtungen, damit die Liste kein Verzeichnis von Stellen wird, an denen früher einmal Farben standen.

Dieselbe Runde hat auch die Namen geradegezogen. Ein Farbfeld ist reine Farbe ohne Text, deshalb gibt es die sechzehn gesprochenen Namen — im großen Farb-Sheet griffen sie nur nicht, dort hießen 43 von 48 Feldern „Eigene Farbe". Das Raster wird aber *erzeugt*, also weiß jedes Feld, aus welchem Farbton es gebaut wurde, und heißt nach seiner Spalte; fünf Rot in einer Spalte sind fünf Rot, ungesagt bleibt nur der genaue Ton. Geraten wird nur noch, wo die Herkunft wirklich fehlt (zuletzt benutzte Farben, Pipette): `nearestPaletteIndex` führt dort den **Farbton**, nicht die Helligkeit — RGB- und Lab-Abstand schicken beide ein blasses Blau zu „Grau", was messtechnisch stimmt und einem Kind nichts sagt.
