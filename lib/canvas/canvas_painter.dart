...
/// Everything that *is* the picture: paper, photo background, trace guide,
/// paint layer, the stroke under the finger, line art, number chips.
///
/// Split out of [CanvasPainter.paint] so the magnifier can show the very
/// same content through its lens. A second copy of the layer order would
/// drift — and it would drift silently, because a lens that shows a slightly
/// older picture still looks like a lens.
///
/// What stays out is what is *about* the drawing rather than part of it: the
/// eyedropper loupe below, and the magnifier itself. Neither belongs inside
/// another lens.
void paintCanvasPicture(
    Canvas canvas, Size size, CanvasController controller) {
  ...
  if (controller.symmetryFolds > 1) {
    _drawSymmetryGuides(canvas, size, controller);
  }
...
void _drawSymmetryGuides(
  ...
  for (var k = 0; k < folds; k++) {
    ...
    canvas.drawLine(center, center + Offset(cos(a), sin(a)) * reach, paint);
  }
}
